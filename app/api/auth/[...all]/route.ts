...
export const POST = async (req: NextRequest) => {
  // Completely disable Arcjet in development
  if (process.env.NODE_ENV === 'development') {
    console.log('Development mode: Skipping Arcjet protection for', req.nextUrl.pathname);
  ...
  if (decision.isDenied()) {
    if (decision.reason.isEmail()) {
      console.warn("Email validation failed for:", req.url);
      return new Response(
    ...
    if (decision.reason.isShield()) {
      console.warn("Shield validation failed for:", req.url);
      return new Response(
